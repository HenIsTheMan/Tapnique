...
namespace Genesis.Wisdom {
	[CustomPropertyDrawer(typeof(ShowHideInInspectorAttribute))]
	internal sealed class ShowHideInInspectorPropertyDrawer: PropertyDrawer {
		public static class ConditionalFieldUtility {
			#region Property Is Visible

			private static object GetVal(SerializedProperty property) {
				switch(property.propertyType) {
					case SerializedPropertyType.String:
						return property.stringValue;
					case SerializedPropertyType.Character:
						return Convert.ToChar(property.intValue);
			...
			public static bool PropertyIsVisible(SerializedProperty property, bool inverse, string[] compareAgainst) {
				...
				string asStr = AsStringValue(property);
				if(property.propertyType != SerializedPropertyType.String) {
					asStr = asStr.ToUpper();

					compareAgainst = compareAgainst.Select((val) => {
						return val.ToUpper();
					}).ToArray();
				}

				if(compareAgainst != null && compareAgainst.Length > 0) {
					var matchAny = CompareAgainstValues(asStr, compareAgainst, IsFlagsEnum());
					if(inverse)
						matchAny = !matchAny;
					return matchAny;
				}

				bool someValueAssigned = asStr != "FALSE" && asStr != "0" && asStr != "NULL";
			...
			public static bool BehaviourPropertyIsVisible(UnityEngine.Object obj, string propertyName, ShowHideInInspectorAttribute appliedAttribute) {
				if(string.IsNullOrEmpty(appliedAttribute.fieldName))
					return true;

				var so = new SerializedObject(obj);
				var property = so.FindProperty(propertyName);
				var targetProperty = FindRelativeProperty(property, appliedAttribute.fieldName);

				return PropertyIsVisible(targetProperty, !appliedAttribute.isShow, appliedAttribute.valStrArr);
			}
		...
		public override float GetPropertyHeight(SerializedProperty property, GUIContent label) {
			if(!(attribute is ShowHideInInspectorAttribute conditional))
				return 0;

			Initialize(property);

			var propertyToCheck = ConditionalFieldUtility.FindRelativeProperty(property, conditional.fieldName);
			_toShow = ConditionalFieldUtility.PropertyIsVisible(propertyToCheck, !conditional.isShow, conditional.valStrArr);
			if(!_toShow)
				return 0;
		...
		private void Initialize(SerializedProperty property) {
			...
			void CacheAllDrawersInDomain() {
				if(!(_allPropertyDrawersInDomain == null || _allPropertyDrawersInDomain.Count == 0))
					return;
				...
				foreach(var type in allDrawerTypesInDomain) {
					...
					if(_allPropertyDrawersInDomain.ContainsKey(associatedType))
						continue;
			...
			void TryGetCustomAttributeDrawer() {
				...
				var secondAttribute = (PropertyAttribute)fieldInfo.GetCustomAttributes(typeof(PropertyAttribute), false)
					.FirstOrDefault(a => !(a is ShowHideInInspectorAttribute));
				if(secondAttribute == null)
					return;
				var genericAttributeType = secondAttribute.GetType();

				//Get the associated attribute drawer
				if(!_allPropertyDrawersInDomain.ContainsKey(genericAttributeType))
					return;
				...
				try {
					...
					if(attributeField != null)
						attributeField.SetValue(_customAttributeDrawer, secondAttribute);
				} catch(Exception e) {
					Debug.LogWarning(
						"Unable to construct drawer for " + secondAttribute.GetType() + " : " + e,
			...
			void TryGetCustomTypeDrawer() {
				...
				try {
					...
					Debug.LogWarning("No constructor available in " + fieldType + " : " + e, property.serializedObject.targetObject);
					return;
				}
				...
				if(attributeField != null)
					attributeField.SetValue(_customTypeDrawer, attribute);
				...
				if(fieldInfoField != null)
					fieldInfoField.SetValue(_customTypeDrawer, fieldInfo);
			}
		}
		...
		public override void OnGUI(Rect pos, SerializedProperty property, GUIContent label) {
			...
			else if(_customTypeDrawer != null)
				TryUseTypeDrawer();
			else
			...
			void TryUseAttributeDrawer() {
				try {
					_customAttributeDrawer.OnGUI(pos, property, label);
				} catch(Exception e) {
					EditorGUI.PropertyField(pos, property, label);
					Debug.LogWarning(
						"Unable to use Custom Attribute Drawer " + _customAttributeDrawer.GetType() + " : " + e,
						property.serializedObject.targetObject
					);
				}
			}
			...
			void TryUseTypeDrawer() {
				try {
					_customTypeDrawer.OnGUI(pos, property, label);
				} catch(Exception e) {
					EditorGUI.PropertyField(pos, property, label);
					Debug.LogWarning(
						"Unable to instantiate " + fieldInfo.FieldType + " : " + e,
						property.serializedObject.targetObject
					);
				}
			}

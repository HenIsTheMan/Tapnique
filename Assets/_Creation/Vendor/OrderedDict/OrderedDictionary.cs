...
namespace External.Common.Collections.Generic {
	...
	public class OrderedDictionary<TKey, TValue>: IOrderedDictionary<TKey, TValue> {
		...
		public void SetItem(int index, TValue value) {
			if(index < 0 || index >= _keyedCollection.Count) {
				UnityEngine.Assertions.Assert.IsTrue(false, $"The index is outside the bounds of the dictionary: {index}");
			}
		...
		public TValue GetValue(TKey key) {
			if(_keyedCollection.Contains(key) == false) {
				UnityEngine.Assertions.Assert.IsTrue(false, $"The given key is not present in the dictionary: {key}");
			}
		...
		int ICollection.Count {
			get {
				return ((ICollection)_keyedCollection).Count;
			}
		}

		bool ICollection.IsSynchronized {
			get {
				return ((ICollection)_keyedCollection).IsSynchronized;
			}
		}

		object ICollection.SyncRoot {
			get {
				return ((ICollection)_keyedCollection).SyncRoot;
			}
		}
	...
	public class DictionaryEnumerator<TKey, TValue>: IDictionaryEnumerator, IDisposable {
		...
		public object Key {
			get {
				return impl.Current.Key;
			}
		}
		public object Value {
			get {
				return impl.Current.Value;
			}
		}
